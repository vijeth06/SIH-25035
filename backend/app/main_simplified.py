"""
Simplified FastAPI application with robust fallback handling.
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting up simplified sentiment analysis API...")
    yield
    logger.info("Shutting down simplified sentiment analysis API...")

# Create FastAPI application
app = FastAPI(
    title="Simplified Sentiment Analysis Engine API",
    description="A robust sentiment analysis API with fallback handling",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
# Import simplified services with fallback handling
try:
...
async def api_health():
    ...
    return {
        ...
        "message": "Simplified sentiment analysis API is running"
    }
...
async def create_word_cloud(payload: WordCloudRequest):
...
@app.post("/api/v1/sentiment-wordcloud", tags=["visualization"])
async def create_sentiment_word_cloud(payload: WordCloudRequest):
    ...
    try:
        ...
        analysis_results = [
            {"sentiment_label": "neutral", "confidence_score": 0.5}
        ...
        logger.error(f"Error in sentiment word cloud generation: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Sentiment word cloud generation failed: {str(e)}"
        )
...
async def root():
    """Root endpoint."""
    return {
        "message": "Welcome to Simplified Sentiment Analysis Engine API",
        "version": "1.0.0",
        "docs": "/docs",
        "redoc": "/redoc",
        "endpoints": [
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Simplified Sentiment Analysis API...")
    print(f"📍 Host: {host}")
    print(f"📍 Port: {port}")
    print(f"📍 Debug: {debug}")
